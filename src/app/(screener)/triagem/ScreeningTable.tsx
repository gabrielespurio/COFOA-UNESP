...
function getStatusBadge(status: string) {
  switch (status) {
    case 'SUBMITTED':
      return <Badge variant="warning">Aguardando Triagem</Badge>;
    default:
      return <Badge variant="info">{status}</Badge>;
  }
}

function WorkScreeningModal({ work, onClose }: { work: any, onClose: () => void }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          <div>
            <h2 style={{ fontSize: '1.25rem', color: '#0f172a', margin: 0, fontWeight: 700 }}>Triagem de Trabalho</h2>
            <p style={{ color: '#64748b', margin: '0.25rem 0 0 0', fontSize: '0.875rem' }}>Verifique os documentos anexados antes de aprovar para a 2ª etapa.</p>
          </div>
          <button 
            onClick={onClose}
            style={{ 
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: '0.25rem' }}>Autor(a)</label>
                    <div style={{ padding: '0.625rem 0.75rem', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '0.875rem', color: '#0f172a' }}>
                      {work.participant?.fullName || 'Não informado'}
                    </div>
                  </div>
                  <div>
            ...
            <div style={{ paddingTop: '1.5rem', borderTop: '1px solid #f1f5f9' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
                {work.identifiedFileUrl && (
                  <a href={work.identifiedFileUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: '#f8fafc', color: '#475569', border: '1px solid #e2e8f0', textDecoration: 'none', borderRadius: '6px', fontWeight: 500, fontSize: '0.875rem', transition: 'all 0.2s' }}>
                ...
                {work.ethicsCommitteeFileUrl && (
                  <a href={work.ethicsCommitteeFileUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: '#f8fafc', color: '#475569', border: '1px solid #e2e8f0', textDecoration: 'none', borderRadius: '6px', fontWeight: 500, fontSize: '0.875rem', transition: 'all 0.2s' }}>
...
export function ScreeningTable({ works, currentUserId }: { works: any[], currentUserId: string }) {
    ...
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ color: 'var(--color-text-secondary)', fontSize: 'var(--font-size-sm)' }}>
          {filteredWorks.length} {filteredWorks.length === 1 ? 'trabalho aguardando triagem' : 'trabalhos aguardando triagem'}
        </div>
      </div>
          ...
          <tbody>
            {paginatedWorks.length === 0 && (
              <tr>
                <td colSpan={5} style={{ textAlign: 'center', color: 'var(--color-text-secondary)', padding: '2rem' }}>
                  Nenhum trabalho na fila de triagem.
                </td>
              </tr>
            )}
            {paginatedWorks.map(work => (
              <tr 
                ...
                title="Clique na linha para realizar a triagem"
              ...
              >
                ...
                <td>{work.participant?.fullName || 'Não informado'}</td>
